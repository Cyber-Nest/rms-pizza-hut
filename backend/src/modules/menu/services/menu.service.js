...
exports.getAllModifierGroups = async () => {
  try {
    return await ModifierGroup.find()
      .populate("options.modifierGroups")
      .sort({ createdAt: -1 })
      .lean();
  } catch (error) {
...
exports.updateModifierGroup = async (id, groupData) => {
  try {
    const group = await ModifierGroup.findByIdAndUpdate(id, groupData, {
      returnDocument: "after",
      runValidators: true,
    });
    if (!group) {
  ...
  try {
    if (query.minimal === "true" || query.minimal === true) {
      return await Product.find()
        .select(
          "_id name price image categoryId productId isActive kitchenLabel displayOrder disabledBranches outOfStockBranches",
        )
...
exports.getBranchProductsList = async (branchId) => {
  try {
    const products = await Product.find()
      .select(
        "_id name price image itemType categoryId productId isActive kitchenLabel displayOrder isOutOfStock disabledBranches outOfStockBranches",
      )
      .populate("categoryId", "name")
      .sort({ displayOrder: 1, name: 1 });
    ...
    if (branchId) {
      ...
      return products.map((p) => {
        ...
        const isDisabledForBranch =
          p.disabledBranches &&
          p.disabledBranches.some((b) => b.toString() === bIdStr);
        const isOutOfStockForBranch =
          p.outOfStockBranches &&
          p.outOfStockBranches.some((b) => b.toString() === bIdStr);
        return {
          ...pObj,
    ...
    logger.error(
      `Menu Service Error: getBranchProductsList - ${error.message}`,
    );
    throw error;
  }
};
...
exports.toggleProductActive = async (id, isActive, branchId) => {
  try {
    ...
    const product = await Product.findByIdAndUpdate(id, updateQuery, {
      ...
      .select(
        "_id name price image itemType categoryId productId isActive kitchenLabel isOutOfStock disabledBranches outOfStockBranches",
      )
      .populate("categoryId", "name");
    ...
    if (branchId) {
      ...
      const isDisabledForBranch =
        product.disabledBranches &&
        product.disabledBranches.some((b) => b.toString() === bIdStr);
      const isOutOfStockForBranch =
        product.outOfStockBranches &&
        product.outOfStockBranches.some((b) => b.toString() === bIdStr);
      return {
        ...pObj,
...
exports.toggleCategoryBranchVisibility = async (
  categoryId,
  branchId,
  isHidden,
) => {
  try {
    ...
    logger.error(
      `Menu Service Error: toggleCategoryBranchVisibility - ${error.message}`,
    );
    throw error;
  }
};

exports.toggleProductBranchVisibility = async (
  productId,
  branchId,
  isHidden,
) => {
  try {
    ...
    logger.error(
      `Menu Service Error: toggleProductBranchVisibility - ${error.message}`,
    );
    throw error;
  }
};
  ...
  try {
    ...
    const feed = {
      ...
      menuItems: products.map((prod) => {
        const isOutOfStockForBranch =
          bIdStr && prod.outOfStockBranches
            ? prod.outOfStockBranches.some((b) => b.toString() === bIdStr) ||
              prod.isOutOfStock
            : prod.isOutOfStock;
